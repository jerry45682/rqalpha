...
def init(context):
    ...
    logger.info("multi-factor strategy initialized with {}".format(Path(config_path or __file__).name))


def _is_suspended_from_bar(bar_dict, order_book_id):
    try:
        return bool(bar_dict[order_book_id].is_suspended)
...
def rebalance(context, bar_dict):
    ...
    logger.info("rebalance date: {}".format(today))

    stock_pool = resolve_stock_pool(config, index_components=index_components)
    stock_pool = filter_tradeable(
        stock_pool,
        is_suspended=lambda order_book_id: _is_suspended_from_bar(bar_dict, order_book_id),
    )
    logger.info("stock pool size after filters: {}".format(len(stock_pool)))

    if config["factors"].get("source") == "baostock":
        factors = fetch_baostock_factors(stock_pool, history_bars)
    else:
    ...
    for order_book_id in targets:
        ...
        logger.info(
            "factor score {} total={:.6f} pe={:.6f} pb={:.6f} roe={:.6f} momentum_60={:.6f}".format(
                order_book_id,
                row["score"],
    ...
    current_positions = [
        position.order_book_id for position in get_positions()
